...
class SungrowModbusWebClient(ModbusBaseClient):
    ...
    def __init__(self, host='127.0.0.1', port=8082, framer=ModbusSocketFramer, **kwargs):
    ...
    async def execute(self, request_obj):
        ...
        request = self.framer.buildPacket(request_obj)
        self.header = request
        ...
        url = f'http://{str(self.dev_host)}/device/getParam?dev_id={dev_id}&dev_type={str(self.dev_type)}&dev_code={str(self.dev_code)}&type=3&param_addr={address}&param_num={count}&param_type={str(param_type)}&token={self.ws_token}&lang=en_us&time123456={str(int(time.time()))}'
        logging.debug(f'Calling: {url}')
        async with aiohttp.request("GET", url) as r:
            logging.debug("HTTP Status code " + str(r.status))
            if str(r.status) == '200':
                self.payload_dict = await r.json(content_type="text/html")
                ...
                if self.payload_dict.get('result_code',0) == 1:
                    ...
                    payload_modbus = ['00', format(request[1], '02x'), '00', '00', '00', format((data_len+3), '02x'), format(request[6], '02x'), format(request[7], '02x'), format(data_len, '02x')]
                    # Attach the data we recieved from HTTP request to the header we created to make a modbus RTU message
                    payload_modbus.extend(modbus_data)
                    #logging.debug("Modbus RTU: " + str(self.payload_modbus))
                    payload_data = bytes.fromhex("".join(payload_modbus))
                    responses = []
                    self.framer.processIncomingPacket(payload_data, responses.append, dev_id)
